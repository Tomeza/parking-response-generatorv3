 ...
 * 重要なキーワードを抽出する関数
 */
function extractKeyTerms(words, fullQuery) {
  // 日本語の一般的なストップワード
...
async function optimizedSearch(query) {
  ...
  if (
    ...
    const luxuryCarResults = await searchLuxuryCars(normalizedQuery);
  ...
  if (
    ...
    const reservationChangeResults = await searchReservationChange(normalizedQuery);
  ...
  if (
    ...
    const internationalResults = await searchInternationalFlight(normalizedQuery);
  ...
  try {
    ...
    results = await prisma.$queryRaw`
      ...
      FROM "Knowledge" k
      WHERE 
        k.question &@~ ${pgroongaQuery}
        OR k.answer &@~ ${pgroongaQuery}
      ORDER BY
        score DESC
      LIMIT 10
    `;
 ...
 * 予約変更に関する専用検索関数
 */
async function searchReservationChange(query) {
  try {
    const results = await prisma.$queryRaw`
      SELECT 
        k.id,
 ...
 * 国際線に関する専用検索関数
 */
async function searchInternationalFlight(query) {
  try {
    const results = await prisma.$queryRaw`
      SELECT 
        k.id,
 ...
 */
function processSearchResults(results, originalQuery) {
